...
> **大会運営ツール（Swiss Ops）** は別アプリとして [`operations/`](./operations/) にあります。
> スイスドロー組み合わせ・順位（タイブレーク）に加え、フロアスタッフ進行画面・オフライン入力を備えた
> 運営側ツールのローカル完結プロトタイプです。詳細は [operations/README.md](./operations/README.md)。
